...
def analyze_failure_overlap(predictions, actuals, optimal_ci, grade, property_type):
    """Analyze how many predicted failures match actual failures - Updated to use optimal CI"""
    n_samples = len(predictions)
    margin = optimal_ci['margin_of_error']
    
    # Get predicted and actual failure masks
    pred_fails = np.zeros(n_samples, dtype=bool)
    actual_fails = np.zeros(n_samples, dtype=bool)
    
    for i in range(n_samples):
        if property_type == 'Tensile Strength [in MPA]':
            pred_lower = predictions[i] - margin
            pred_upper = predictions[i] + margin
            pred_fails[i] = not meets_requirements_with_intervals(pred_lower, pred_upper, actuals[i], grade, property_type)
        else:
            pred_fails[i] = not meets_requirements(predictions[i] - margin, actuals[i], grade, property_type)
        actual_fails[i] = not meets_requirements(actuals[i], actuals[i], grade, property_type)
    
    # Calculate overlap statistics
    correct_fail_predictions = np.sum(pred_fails & actual_fails)
    missed_fails = np.sum(~pred_fails & actual_fails)
    false_alarms = np.sum(pred_fails & ~actual_fails)
    
    total_actual_fails = np.sum(actual_fails)
    total_pred_fails = np.sum(pred_fails)
    
    return {
        'correct_fails': correct_fail_predictions,
        'correct_fail_rate': (correct_fail_predictions/total_actual_fails * 100) if total_actual_fails > 0 else 0,
        'missed_fails': missed_fails,
        'false_alarms': false_alarms,
        'total_actual_fails': total_actual_fails,
        'total_pred_fails': total_pred_fails
    }

def analyze_classification_results(metrics, optimal_cis):
    """
    Analyze and print classification results with debug information
    """
    grades = ['S355', 'S690']
    properties = ['Yield Strength [Mpa]', 'Tensile Strength [in MPA]', 'Elongation']
    categories = ['PASS', 'SEMI-PASS', 'FAIL']
    
    # Print confidence intervals being used
    print("\n=== Confidence Intervals ===")
    for grade in grades:
        print(f"\n{grade} Confidence Intervals:")
        for prop in properties:
            margin = optimal_cis[grade][prop]['margin_of_error']
            print(f"{prop}: ±{margin:.2f}")
    
    results_data = {'Predicted': {}, 'Actual': {}}
    
    # Debug sample
    debug_sample_index = 0  # Look at first sample for debugging
    print("\n=== Debug Sample Analysis ===")
    
    for grade in grades:
        print(f"\n{grade} Sample {debug_sample_index}:")
        for prop in properties:
            pred = metrics[grade]['predictions'][prop][debug_sample_index]
            actual = metrics[grade]['actual_values'][prop][debug_sample_index]
            margin = optimal_cis[grade][prop]['margin_of_error']
            
            # Calculate confidence interval using absolute margin
            ci_lower = pred - margin
            ci_upper = pred + margin
            
            print(f"\n{prop}:")
            print(f"Predicted: {pred:.2f}")
            print(f"Actual: {actual:.2f}")
            print(f"Confidence Interval: [{ci_lower:.2f}, {ci_upper:.2f}] (±{margin:.2f})")
            
            # Get requirements
            requirements = get_min_requirements(16, grade)
            if prop == 'Yield Strength [Mpa]':
                req_min = requirements['yield_strength']['min']
                req_max = requirements['yield_strength']['max']
                print(f"Requirements - Min: {req_min}, Max: {req_max if req_max else 'None'}")
            elif prop == 'Tensile Strength [in MPA]':
                req_min = requirements['tensile_strength']['min']
                req_max = requirements['tensile_strength']['max']
                print(f"Requirements - Min: {req_min}, Max: {req_max if req_max else 'None'}")
            elif prop == 'Elongation':
                req_min = requirements['elongation']['min']
                req_max = requirements['elongation']['max']
                print(f"Requirements - Min: {req_min}, Max: {req_max if req_max else 'None'}")
...
def main():
    ...
    analyze_classification_results(metrics, optimal_cis)
